...
// Disjoint matrix blocks across threads → safe to share the pointer.
...
// -------- optional: per-sample branch coverage logging --------
...
// -------- main striped UniFrac routine (unchanged core) --------
...
fn main() -> Result<()> {
    ...
    // Build masks from input (TSV or BIOM)
    let (taxa, mut samples, pres_dense);
    let mut pres = Vec::<Vec<f64>>::new(); // TSV
    let mut indptr = Vec::<u32>::new();    // BIOM
    let mut indices = Vec::<u32>::new();   // BIOM
    ...
    // ---- Drop empty samples (all-zero taxa) before computing UniFrac ----
    let mut keep_idx = Vec::with_capacity(nsamp);
    for s in 0..nsamp {
        // Treat as empty if no leaves present at all
        let is_empty = masks[s].as_raw_slice().iter().all(|&w| w == 0);
        if is_empty {
            warn!("Dropping sample '{}' (no taxa present; all zeros).", samples[s]);
        } else {
            keep_idx.push(s);
        }
    }
    if keep_idx.len() != nsamp {
        let kept = keep_idx.len();
        let mut new_masks = Vec::with_capacity(kept);
        let mut new_names = Vec::with_capacity(kept);
        for i in keep_idx {
            new_masks.push(masks[i].clone());
            new_names.push(samples[i].clone());
        }
        masks = new_masks;
        samples = new_names;
        nsamp = samples.len();
